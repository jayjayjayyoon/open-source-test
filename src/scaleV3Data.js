// Synthetic data only. Never substitute these fixtures for actual project records.
...
export function createScaleV3Data(size, scenario = 'balanced') {
  ...
  const nodes = [{ id: 'root', parentId: null, kind: 'root', title: '전체 작업 영역', area: -1, project: -1, status: 'NOW', x: 40, y: 1000 }];
  for (let index = 0; index < AREAS.length; index += 1) {
    nodes.push({ id: `area-${index}`, parentId: 'root', kind: 'area', title: AREAS[index], area: index, project: -1, status: STATUSES[index % 4], x: 320, y: index * 430 + 100 });
  }
  for (let index = 0; index < projectCount; index += 1) {
    const area = Math.floor(index * AREAS.length / projectCount);
    nodes.push({ id: `project-${index}`, parentId: `area-${area}`, kind: 'project', title: `예시 프로젝트 ${String(index + 1).padStart(3, '0')}`, area, project: index, status: STATUSES[index % 4], x: 630, y: index * 112 + 60 });
  }
  ...
  for (let index = 0; index < childrenCount; index += 1) {
    ...
    const titlePrefix = { task: '가상 작업', document: '가상 문서', decision: '가상 결정' }[kind];
    nodes.push({ id: `item-${index}`, parentId: `project-${project}`, kind, title: `${titlePrefix} ${String(index + 1).padStart(4, '0')}`, area, project, status: STATUSES[index % 4], x: 990 + Math.floor(sequence / 4) * 185, y: project * 112 + (sequence % 4) * 24 + 70 });
  }
  ...
  if (scenario === 'relations' || scenario === 'mixed') {
    for (let index = 0; index < projectCount; index += 1) {
      const other = (index + 7) % projectCount;
      edges.push({ id: `related-${index}`, source: `project-${index}`, target: `project-${other}`, relation: 'related' });
    }
  }
...
// Show only the requested branch; related links appear only when both endpoints are present.
export function selectScaleV3Graph(data, mode, area = null, project = null, page = 0) {
  if (mode === 'full') return data;
  const allowedProjects = area === null ? [] : areaProjects(data, area).slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);
  const projectIds = new Set(allowedProjects.map((node) => node.id));
  const visible = data.nodes.filter((node) => node.kind === 'root' || node.kind === 'area' || (node.kind === 'project' && projectIds.has(node.id)) || (project !== null && node.parentId === `project-${project}`));
